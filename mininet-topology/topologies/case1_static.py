...
class StaticTopo(Topo):
    def build(self):
        ...
        bw_core = 10
        self.addLink(s0, cloud, bw=bw_core)  # s0-eth1
        ...
        for i in range(1,4):
            ...
            self.addLink(s1, h, bw=bw_host)
        for i in range(4,6):
            h = self.addHost(f"h{i}", ip=f"10.0.2.{i}/24")
            self.addLink(s2, h, bw=bw_host)
        for i in range(6,8):
            h = self.addHost(f"h{i}", ip=f"10.0.3.{i}/24")
            self.addLink(s3, h, bw=bw_host)
        for i in range(8,11):
            h = self.addHost(f"h{i}", ip=f"10.0.4.{i}/24")
            self.addLink(s4, h, bw=bw_host)

def run():
    ctrl_ip = os.environ.get("CONTROLLER_IP", "ryu-controller")
    ctrl_port = int(os.environ.get("CONTROLLER_PORT", "6633"))
    run_seconds = int(os.environ.get("RUN_SECONDS", "90"))
    start_traffic = os.environ.get("START_TRAFFIC", "1") == "1"
    ...
    cloud.cmd("python3 /traffic-generator/iot_server.py --bind 0.0.0.0 --out /shared/raw/case1_server.csv &")
    time.sleep(1)
    ...
    if start_traffic:
        for i in range(1,11):
            ...
            h.cmd(f"python3 /traffic-generator/iot_sensor.py --name h{i} --server {cloud.IP()} --case case1 --out /shared/raw/case1_client_h{i}.csv &")
