...
wait_mininet() {
  if docker ps -a --format '{{.Names}}' | grep -qx 'mininet'; then
    echo "[wait] waiting for mininet to finish..."
    docker wait mininet >/dev/null || true
  else
    echo "[wait] mininet container not found (did compose start correctly?)"
    return 1
  fi
}
